; (function (root, factory) {
  ...
  function init(indexFileId) {
    ...
    function addToDB(entry, { dbMain, dbFragment }) { // This function has been changed to allowed for update
      const { properties } = INDEX[dbMain];
      const { cumulative } = properties
      const { key, id } = entry;
      if (!dbMain) {
        console.log("Missing DBMain!")
        return null
      }
      const foundFragement = getFragmentFromQuerry(key, "keyQueryArray", dbMain)
      dbFragment = foundFragement || getProperFragment(dbMain, dbFragment);
      if (cumulative && !foundFragement) dbFragment = checkOpenDBSize(dbMain, dbFragment);
    ...
    function createNewCumulativeFragment(dbMain, dbFragment) {
      const { properties } = INDEX[dbMain];
      const { filesPrefix } = properties;
      const lastDBFragment = dbFragment || getLastCreatedFragment(dbMain);
      const countingRegex = /_\d/g
      let newFragment
      if (!lastDBFragment) newFragment = filesPrefix + "_" + dbMain + "_1"; // add proper prefix here
